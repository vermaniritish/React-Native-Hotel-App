...
const FlightsCalendarListScreen = (props) => {
    ...
    const onDayPress = (day) => {
        ...
        if (
            ...
            (
                (
                    endDay && !props.rangePicker 
                    && 
                    moment(startDay).unix() <= moment(day.dateString).unix()
                ) 
                ||
                (
                    !endDay
        ...
        else if (startDay && endDay && props.rangePicker && moment(endDay).unix() > moment(day.dateString).unix()) {
            const date = {}
        ...
        else if (startDay && !endDay && props.rangePicker && moment(startDay).unix() == moment(day.dateString).unix()) {
            const date = {}
            for (const d = moment(startDay); d.isSameOrBefore(day.dateString); d.add(1, 'days')) {
                date[d.format('YYYY-MM-DD')] = {
                    marked: false,
            ...
            >
                <View
                    style={[
                        ...
                        {paddingTop: hp('8'), paddingBottom: hp('10'), width: '100%',width: wp('56')}, 
                        prop.marking && prop.marking.selected ? {width: wp('56'),paddingHorizontal: hp('10'), backgroundColor: '#3171c9', textAlign: 'center'} : {},
...
export default connect(mapStateToProps, mapDispatchToProps)(FlightsCalendarListScreen);
